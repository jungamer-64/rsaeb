...
/// Program-local zero-based position of a parsed rule.
///
/// This is metadata, not a lookup capability. Rule metadata is borrowed directly
/// from [`Program::rules`] or trace events so callers cannot manufacture a stale
pub struct Program {
...
impl RunResult {
    fn stable(output: Vec<u8>, steps: usize) -> Self {
        Self {
            output,
            steps,
    ...
    fn from_return(output: Vec<u8>, steps: usize) -> Self {
        Self {
            output,
            steps,
